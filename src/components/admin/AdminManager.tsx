  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-200 dark:border-zinc-800 pb-3">
        <div>
          <h1 className="text-lg md:text-xl font-bold text-zinc-900 dark:text-zinc-50 tracking-tight leading-tight">Admin authorizations</h1>
          <p className="text-[10px] text-zinc-500 dark:text-zinc-400 mt-0.5">Configure dashboard access permissions, invite team members, and manage credential safety.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-[#121517] border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl p-4 shadow-sm space-y-4">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-red-500/10 dark:bg-red-550/15 rounded-xl flex items-center justify-center">
                <UserPlus className="w-4 h-4 text-red-500" />
              </div>
              <div>
                <h2 className="text-xs font-black text-zinc-900 dark:text-zinc-50 uppercase tracking-wider">Invite Admin</h2>
                <p className="text-[9px] text-zinc-450 dark:text-zinc-500">Authorize database access</p>
              </div>
            </div>
            
            <form onSubmit={handleAddAdmin} className="space-y-3">
              <div>
                <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-550 mb-1">Email Address</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-50 dark:bg-[#0c0e10] border border-zinc-200 dark:border-zinc-800 rounded-lg pl-8.5 pr-3.5 py-1.5 text-xs text-zinc-900 dark:text-zinc-50 focus:outline-none focus:ring-2 focus:ring-red-500/30 font-medium"
              ...
              <div>
                <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-550 mb-1">Password</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-zinc-50 dark:bg-[#0c0e10] border border-zinc-200 dark:border-zinc-800 rounded-lg pl-8.5 pr-9 py-1.5 text-xs text-zinc-900 dark:text-zinc-50 focus:outline-none focus:ring-2 focus:ring-red-500/30 font-medium"
                    placeholder="Min. 6 characters"
                  />
                  <button 
                    type="button"
                    onClick={() => setShowPass(!showPass)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100"
                  >
              ...
              <div>
                <label className="block text-[9px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-550 mb-1">Confirm Password</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-400" />
                  <input 
                    type={showPass ? "text" : "password"}
              ...
              <button 
                ...
                className="w-full py-1.5 bg-red-650 hover:bg-red-750 text-white rounded-lg font-bold text-xs uppercase tracking-widest transition-all disabled:opacity-50 flex items-center justify-center gap-1.5 mt-1 shadow-md shadow-red-500/10"
              >
                {isAdding ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ShieldCheck className="w-3.5 h-3.5" />}
                Authorize Admin
              </button>
            </form>

            <div className="pt-3 border-t border-zinc-100 dark:border-zinc-850">
              <div className="flex items-start gap-2 p-3 bg-red-500/5 dark:bg-red-550/5 border border-red-500/10 rounded-xl">
                <AlertTriangle className="w-4 h-4 text-red-550 flex-shrink-0 mt-0.5" />
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white dark:bg-[#121517] border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-150 dark:border-zinc-850 flex justify-between items-center bg-zinc-50/50 dark:bg-[#101315]/20">
              <h3 className="font-bold text-xs text-zinc-900 dark:text-zinc-50 flex items-center gap-1.5">
                <Shield className="w-3.5 h-3.5 text-red-550" />
                Authorized Workspace Admins
              </h3>
              <span className="text-[9px] font-black uppercase tracking-wider text-zinc-400 dark:text-zinc-500 bg-zinc-100 dark:bg-[#0c0e10] border border-zinc-200/70 dark:border-zinc-800/80 px-2 py-0.5 rounded-full">
            ...
            <div className="divide-y divide-zinc-150 dark:divide-zinc-850">
              {loading ? (
                ...
                <div className="p-12 text-center text-zinc-450 dark:text-zinc-500 font-semibold italic text-xs">No editors currently authorized inside the workspace.</div>
              ) : admins.map((admin) => {
                const role = getAdminRole(admin.email, user?.email);
                return (
                  <div key={admin.id} className="p-3 py-2.5 flex items-center justify-between group hover:bg-zinc-50/50 dark:hover:bg-[#101315]/10 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="w-8.5 h-8.5 bg-zinc-50 dark:bg-[#0c0e10] border border-zinc-150 dark:border-zinc-800 rounded-xl flex items-center justify-center text-zinc-450 dark:text-zinc-500 group-hover:bg-red-500/10 group-hover:text-red-500 transition-all">
                        <User className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="flex flex-wrap items-center gap-1.5">
                          <span className="font-bold text-xs text-zinc-900 dark:text-zinc-50">{admin.email}</span>
                          <span className={`px-1.5 py-0.25 rounded text-[8px] font-black uppercase tracking-wider border ${role.style}`}>
                            {role.name}
                          </span>
                        </div>
                        <p className="text-[9px] text-zinc-450 dark:text-zinc-500 font-semibold mt-0.5 flex items-center gap-1.5">
                    ...
                    {user?.email !== admin.email && (
                      <button 
                        ...
                        className="p-1.5 text-zinc-400 hover:text-red-550 hover:bg-red-500/5 dark:hover:bg-red-500/10 rounded-lg transition-all"
